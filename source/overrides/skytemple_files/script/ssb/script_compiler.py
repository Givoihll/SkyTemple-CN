...
import re
_UNESCAPE_RE = re.compile(r'_X\{([0-9A-Fa-f]{4})\}')
def _unescape(s):
    if s:
...
class ScriptCompiler:
    ...
    def compile_structured(
        ...
        if len(routine_ops) > 0:
            ...
            input_routine_structure: list[tuple[SsbRoutineInfo, str, list[SsbOperation]]] = list(
                zip(routine_infos, named_coroutines, routine_ops)
            )
    ...
    def _parse_param(
        ...
        if isinstance(param, SsbOpParamConstString):
            try:
                ...
                built_constants.append(_unescape(param.name))
                return i
        ...
        if isinstance(param, SsbOpParamLanguageString):
            ...
            if len(param.strings.keys()) == 1:
                ...
                for lang in built_strings.keys():
                    built_strings[lang].append(_unescape(only_value))
            else:
                ...
                for lang, string in param.strings.items():
                    ...
                    built_strings[lang].append(_unescape(string))

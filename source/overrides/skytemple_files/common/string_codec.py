...
# Custom character set for Chinese/CJK support
# Loaded from charmap.txt at runtime
_custom_encode_table: dict[str, bytes] = {}  # str char -> 2-byte big-endian
_custom_decode_table: dict[int, str] = {}    # int code -> str char
_charmap_loaded = False


def load_custom_charset(charmap_path: str) -> int:
    """
    Load a charmap.txt file for custom character encoding.
    Format: each line is HEX=CHAR, e.g. "922C=水"
    Returns number of characters loaded.
    """
    global _custom_encode_table, _custom_decode_table, _charmap_loaded
    ...
    with open(charmap_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if m:
                ...
                char = m.group(2)
                if code > 0xFF:  # Only use 2-byte codes
                    hi = (code >> 8) & 0xFF
                    lo = code & 0xFF
                    _custom_encode_table[char] = bytes([hi, lo])
                    _custom_decode_table[code] = char
                    count += 1
    _charmap_loaded = True
    return count
...
def pmd2_encode(text: str, *args: list[Any]) -> tuple[bytes, int]:
    ...
    cursor = 0
    for c in text:
        if c in _custom_encode_table:
            b = _custom_encode_table[c]
            bytearr[cursor:cursor+len(b)] = b
            cursor += len(b)
        elif c in _encode_table:
    ...
    while i < len(binary):
        ...
        if current_is_special_character:
            if x in _decode_shift_jis_characters:
                result += _decode_shift_jis_characters[x]
            else:
                raise ValueError(
            ...
            current_is_special_character = False
            i += 1
        elif 0x81 <= x <= 0x9F and i + 1 < len(binary):
            # Check for custom 2-byte CJK character
            code = (x << 8) | binary[i + 1]
            if code in _custom_decode_table:
                result += _custom_decode_table[code]
                len_str -= 1  # 2 bytes = 1 character
                i += 2
                continue
            # Not custom, fall through to normal handling
            if x == 129:
                current_is_special_character = True
                len_str -= 1
                i += 1
            elif x in _decode_table:
                result += _decode_table[x]
                i += 1
            else:
                raise ValueError(
